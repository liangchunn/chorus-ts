...
client.on('interactionCreate', async (interaction: Interaction) => {
  ...
  if (interaction.commandName === 'play') {
    ...
    // Extract the video URL / search term from the command
    const searchTerm = interaction.options.get('song')!.value! as string
    ...
    try {
      // Attempt to create a Track from the user's video URL / searchTerm
      const track = await Track.from(searchTerm, {
        onStart() {
          interaction
...
void client.login(process.env.DISCORD_AUTH_TOKEN)
